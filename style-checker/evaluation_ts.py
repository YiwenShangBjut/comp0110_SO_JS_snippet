...
csv_js_name='output-'+str(num)+'k-ts-file.csv'
...
with open(csv_all_name,'r',encoding='utf-8', errors='ignore') as csv_file:
    ...
    for line in csv_reader:
        ...
        if line[2] == "Eslint ERROR" and line[1]!='typescript':
...
# print(jquery_error_list)
# print(js_error_list[0])
standard_num=0
jquery_num=0
not_same_error=0
for i in range(0, len(both_error_list)):
    if jquery_error_list[i] != js_error_list[i]:
        ...
        # print(jquery_error_list[i])
        if type_list[i]=='standard':
            standard_num+=1
        else:
            jquery_num+=1
print(not_same_error)
print('exact same result in standard:', standard_num)
print('exact same result in jquery:', jquery_num)
