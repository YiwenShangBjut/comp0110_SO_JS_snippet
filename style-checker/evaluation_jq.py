...
csv_jq_name='output-'+str(num)+'k-jq-file.csv'
...
with open(csv_all_name,'r',encoding='utf-8', errors='ignore') as csv_file:
    ...
    for line in csv_reader:
        ...
        if line[2] == "Eslint ERROR" and line[1]=='standard':
            javascript_EE_list.append(line[0])
        if line[2] != "Eslint ERROR" and line[1]=='standard':
            jquery_list.append(line[0])

js_num=0
with open(csv_jq_name,'r',encoding='utf-8', errors='ignore') as csv_file:
    csv_reader = csv.reader(csv_file)
    for line in csv_reader:
        js_num+=1
        if line[2]=="Eslint ERROR":
            if line[0] not in javascript_EE_list:
                EE_in_qs_not_in_js.append(line[0])
        else:
            if line[0] in javascript_EE_list:
...
print('number of javascript: '+str(js_num))
